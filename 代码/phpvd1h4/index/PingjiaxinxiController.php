<?php
session_start();
class PingjiaxinxiController extends CommonController {
    ...
    public function page(){
		...
		$where = " where 1 ";//查询条件
            ...
            $where .= " and `gerenzhanghao` = '".$tokens['username']."' ";
        }
                                                                if($tokens['isAdmin'] != 1 && $tokens['tablename'] == 'paotuiyuan') {
            $where .= " and `paotuixingming` = '".$tokens['username']."' ";
        }
        ...
        if ($base['isAdmin']!=1){
            ...
            if (isset($_SESSION[$md5]) && in_array($colum, $columData)){
                if($base['tablename'] == 'yonghu'){
                    $where .= " and `gerenzhanghao` = '".$_SESSION[$md5]."'";
                }
            }
        }
                                                                $base = json_decode(base64_decode($token),true);
        if ($base['isAdmin']!=1){
            $md5 = md5($userid."+10086");
            $colum = "paotuixingming";
            $columData = $base['columData'];
            if (isset($_SESSION[$md5]) && in_array($colum, $columData)){
                if($base['tablename'] == 'paotuiyuan'){
                    $where .= " and `paotuixingming` = '".$_SESSION[$md5]."'";
                }
            }
        }
                                                                        		        
                                        $sql = "select * from `pingjiaxinxi` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $startCount = ($page-1)*$limt;
                    $lists = "select * from `pingjiaxinxi` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function lists(){
        ...
        if ($base['isAdmin']!=1){
            ...
            if (isset($base['success'][$colum]) && in_array($colum, $columData)){
                $where .= " and `gerenzhanghao` = '".$base['success'][$colum]."'";
            }
        }
                                                                $token = $this->token();
        $base = json_decode(base64_decode($token),true);
        ...
        if ($base['isAdmin']!=1){
            ...
            $columData = $base['columData'];
            if (isset($base['success'][$colum]) && in_array($colum, $columData)){
                $where .= " and `paotuixingming` = '".$base['success'][$colum]."'";
            }
        }
                                                                        		                                $sql = "select * from `pingjiaxinxi`".$where;
        $count = table_sql($sql);
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
                ...
                $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `pingjiaxinxi` (".$k.") VALUES (".$v.")";
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE pingjiaxinxi SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function delete(){
        ...
        $sql = "delete from pingjiaxinxi WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                        ...
                        $sql = "select * from `pingjiaxinxi` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function remind($columnName,$type){
        ...
        $sql = "select * from `pingjiaxinxi` where 1 ".$where;
        $result = table_sql($sql);
