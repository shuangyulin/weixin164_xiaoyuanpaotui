...
class YonghuController extends CommonController {
    ...
    public function login(){
        ...
        $password = isset($_REQUEST['password'])?$_REQUEST['password']:"";
                        $sql = "select * from `yonghu` where `gerenzhanghao` = '".$username."' and `mima` = '".$password."'";
    ...
    public function register(){
        ...
        $postData = json_decode($tmpData,true);
                        $colum = "gerenzhanghao";
        $trues = "select * from `yonghu` where `gerenzhanghao` = '".$postData[$colum]."'";
    ...
    public function resetPass(){
        $username = $_REQUEST['username'];
                        $counts = "select * from `yonghu` where gerenzhanghao = '".$username."'";
        $cotte = table_sql($counts);
        if($cotte->num_rows<1){
            exit(json_encode(['code'=>500,'msg'=>"用户名错误。"]));
        }
        $sql = "update yonghu set mima = '123456' where gerenzhanghao = '".$username."'";
                                                                                                                        		        $result = table_sql($sql);
